...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    setTimeout(() => {
      ...
      router.push(`/results?jobId=MOCK-JOB-${Date.now().toString().slice(-4)}`);
    }, 1500);
  };
  ...
  return (
    ...
    >
      <motion.div variants={{ hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0 } }}>
        ...
        <p className="text-stone-500 mt-1 text-sm">Upload CSV files or an entire folder of CSVs for bulk processing.</p>
      </motion.div>
      ...
      <motion.div variants={{ hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0 } }}>
        <div 
          className={`border-2 border-dashed rounded-3xl p-12 text-center transition-all ${
            isDragging ? "border-[#9670f8] bg-[#f7f5ff]" : "border-stone-200 glass-card hover:bg-white/80 hover:border-stone-300"
          }`}
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-stone-900 mb-2 tracking-tight">Drag & Drop CSV files or folders</h3>
          <p className="text-stone-500 mb-8 max-w-sm mx-auto leading-relaxed text-sm">
            You can drop multiple files here at once, or click below to browse.
          </p>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <label 
              htmlFor="csv-upload"
              className="cursor-pointer secondary-action"
            >
              Browse Files
            </label>
            ...
            <label 
              ...
              className="cursor-pointer primary-action"
            >
              Upload Folder
            </label>
          </div>
      ...
      {files.length > 0 && (
        <motion.div variants={{ hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0 } }} className="glass-card p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-semibold text-stone-900">{files.length} File{files.length !== 1 && 's'} Selected</h3>
            <button 
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
            {files.map((f, i) => (
              <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-[rgba(247,245,255,0.5)] border border-white/40 hover:bg-white/80 transition-colors group">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-medium text-stone-500 uppercase tracking-wide">{(f.size / 1024).toFixed(2)} KB</p>
                  </div>
                </div>
                <button 
